# web_scanner.py (versión optimizada para velocidad y compatibilidad con Docker/WSL)
import socket
import nmap
import csv
import sqlite3
...
def nslookup(ip):
    ...
    except (socket.herror, socket.gaierror, UnicodeError, Exception):
        return ''

def check_single_port(ip, port):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.settimeout(0.3)  # ⬅️ REDUCIDO DE 1s A 0.3s PARA MAYOR VELOCIDAD
    try:
...
def parse_nmap_output(nm, ip):
    ...
    if ip not in nm.all_hosts():
        return ports
    # usa get(...) para evitar KeyError si no hay 'tcp' u otro proto
    for proto in nm[ip].all_protocols():
        proto_dict = nm[ip].get(proto, {})
        for p_str, info in proto_dict.items():
            try:
                p = int(p_str)
            except Exception:
                continue
            state = info.get('state', 'unknown')
            service = info.get('name', info.get('product', 'unknown'))
            ports.append({'port': p, 'state': state.upper(), 'service': service})
    return ports
...
# --- Exporters (sin cambios, ya son eficientes) ---
...
def scan_ips(ips, specific_port=10050, run_nmap_scan=False, full_nmap_scan=False):
    ...
    for ip in ips:
        ...
        entry = {'ip': ip, 'hostname': hostname, 'ports': []}

        # Puerto específico
        if specific_port is not None:
            state = check_single_port(ip, specific_port)
            service = 'zabbix-agent' if specific_port == 10050 else 'unknown'
            entry['ports'].append({'port': specific_port, 'state': state, 'service': service})
            save_to_db(ip, hostname, specific_port, state, service)

        # Nmap (si se solicita)
        if run_nmap_scan or full_nmap_scan:
            port_range = '1-65535' if full_nmap_scan else '1-1024'
            # argumentos sugeridos: T4 para velocidad, -sT para no necesitar root
            args = '-Pn -sT -T4 --max-retries 1 --host-timeout 15s --min-rate 500'
            try:
                print(f"[+] Ejecutando nmap: nmap {args} -p {port_range} {ip}")
                nm.scan(hosts=ip, ports=port_range, arguments=args)

                # DEBUG: ver stats/command_line
                try:
                    print("[nmap command]", nm.command_line())
                    print("[nmap stats]", nm.scanstats())
                except Exception:
                    pass
                ...
                for p in ports:
                    ...
                    save_to_db(ip, hostname, p['port'], p['state'], p['service'])
            except nmap.PortScannerError as e:
                print(f"[!] nmap error para {ip}: {e}")
            except Exception as e:
                print(f"[!] Error inesperado en nmap para {ip}: {e}")
